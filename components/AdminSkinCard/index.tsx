...
interface AdminSkinCardProps {
  skinData: SkinData
}

export default function AdminSkinCard({ skinData }: AdminSkinCardProps) {
  const [isEditing, setIsEditing] = useState(false)
  const [editedDiscountPrice, setEditedDiscountPrice] = useState(
    skinData.discount_price,
  )
  const [editedIsVisible, setEditedIsVisible] = useState(skinData.is_visible)

  const updateInventory = useUpdateInventory()

  const priceInReais = parseFloat(skinData.price)
  const discountPrice = parseFloat(
    editedDiscountPrice || skinData.discount_price,
  )

  const handleEdit = () => {
    setIsEditing(true)
    setEditedDiscountPrice(skinData.discount_price)
    setEditedIsVisible(skinData.is_visible)
  }
  ...
  const handleSave = () => {
    ...
    if (editedDiscountPrice !== skinData.discount_price) {
      updates.discount_price = editedDiscountPrice
    }

    if (editedIsVisible !== skinData.is_visible) {
      updates.is_visible = editedIsVisible
    }
    ...
    updateInventory.mutate(
      ...
      },
      {
        ...
        onError: (error) => {
          toast.error(`Erro ao atualizar skin: ${error.message}`)
        },
      },
    )
  }
    ...
    <div className="flex justify-center">
      ...
      >
        ...
        <Tooltip>
          <TooltipTrigger>
            {skinData.discount < 0 && (
              <Info className="absolute right-0 z-20 mr-4 mt-8" />
            )}
            {skinData.discount > 24 && (
              <BadgePercent className="absolute right-0 z-20 mr-4 mt-8" />
            )}
          </TooltipTrigger>
          <TooltipContent>
        ...
        <div className="mt-4 flex flex-1 flex-col px-3">
          ...
          <div className="mt-4 space-y-3">
            <div className="flex items-center justify-between">
              <label className="text-sm font-medium">Visível na loja:</label>
              <VisibilityToggle
                isVisible={editedIsVisible}
                onToggle={setEditedIsVisible}
                disabled={!isEditing}
              />
            </div>
              ...
              {isEditing ? (
                <div className="flex gap-2">
                  <Button
                    ...
                    className="flex flex-1 items-center justify-center gap-2 bg-green-600 hover:bg-green-700 text-white"
                  >
                ...
                <Button
                  onClick={handleEdit}
                  className="flex w-full items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white cursor-pointer"
                >
                  <Edit3 size={16} />
                  Editar
                </Button>
